...
if obesity_file is not None and malnutrition_file is not None:
    ...
    st.sidebar.subheader("📊 Obesity Data")
    st.sidebar.dataframe(df_obesity)

    st.sidebar.subheader("📊 Malnutrition Data")
    st.sidebar.dataframe(df_malnutrition)
    ...
    line_col = st.selectbox("Select numeric column for line plot", df_obesity.select_dtypes(include='number').columns)
    fig2, ax2 = plt.subplots(figsize=(8, 5))
    df_obesity[line_col].plot(ax=ax2)
    st.pyplot(fig2)
...
if selected == 'Queries':  
                    ...
                    if options == " 1.Top 5 regions with the highest average obesity levels in the most recent year(2022)":
                                            ...
                                            cur.execute('SELECT age_group, AVG(CI_Width) AS avg_CI FROM malnutrition GROUP BY age_group ORDER BY avg_CI_width DESC')
                                            result = cur.fetchall()
                                            columns = [desc[0] for desc in cur.description]
                                            data = pd.DataFrame(result,columns=columns)
                                            st.dataframe(data)
                                            ...
                                            cur.execute('SELECT o.country, MAX(o.mean_estimate) - MIN(o.mean_estimate) AS obesity_increase, MIN(m.mean_estimate) - MAX(m.mean_estimate) AS malnutrition_decrease FROM obesity o JOIN malnutrition m ON o.country = m.country GROUP BY o.country HAVING obesity_increase > 0 AND malnutrition_decrease > 0')
                                            result = cur.fetchall()
                                            columns = [desc[0] for desc in cur.description]
                                            data = pd.DataFrame(result,columns=columns)
                                            st.dataframe(data)
